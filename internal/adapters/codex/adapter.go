...
// Starter launches the codex agent asynchronously. It defaults to runner.Start
...
func (a *Adapter) Start(ctx context.Context, request shared.StartRequest) (*shared.Execution, error) {
	...
	if err != nil {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "prepare codex output path", err)
	}

	args := buildExecArgs(execArgsParams{
		WorkingDir:      adapterutil.CommandDir(request.WorkingDir),
	...
	emitPromptBanner(a.liveSink, adapterutil.AgentPrompt(request))
	...
	if startErr != nil {
		...
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "start codex exec", startErr)
	}
...
func interruptFromTerminal(terminal *shared.Execution) (*shared.Execution, error) {
	if terminal == nil {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "codex execution session not found", nil)
	}
...
func (a *Adapter) Resume(ctx context.Context, request shared.ResumeRequest) (*shared.Execution, error) {
	...
	args := buildExecArgs(execArgsParams{
		WorkingDir:      adapterutil.CommandDir(resumeStart.WorkingDir),
		LastMessagePath: lastMessageResult.path,
		Sandbox:         a.sandbox,
		Model:           a.model,
	...
	emitPromptBanner(a.liveSink, adapterutil.AgentPrompt(resumeStart))
	...
	if startErr != nil {
		lastMessageResult.remove()
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "resume codex exec", startErr)
	}
...
func (a *Adapter) binaryPath() (string, error) {
	...
	if errors.Is(err, exec.ErrNotFound) {
		return "", adapterutil.AdapterError(shared.ErrorCodeExecution, "codex binary not found", err)
	}

	return "", adapterutil.AdapterError(shared.ErrorCodeExecution, "locate codex binary", err)
}
...
func (a *Adapter) logPath(request shared.StartRequest) string {
	...
	return filepath.Join(a.logDir, adapterutil.SanitizeID(request.AttemptID)+"-codex.log")
}
...
func (a *Adapter) lookupSession(handle shared.ExecutionHandle) (*agentSession, error) {
	...
	if record.handle.RunID != handle.RunID || record.handle.StepID != handle.StepID || record.handle.AttemptID != handle.AttemptID {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "codex execution handle does not match session", nil)
	}
...
func (a *Adapter) collectTerminal(record *agentSession) (*shared.Execution, error) {
	record.once.Do(func() {
		...
		if parseErr != nil {
			record.buildErr = adapterutil.AdapterError(shared.ErrorCodeResult, "parse codex json output", parseErr)
			return
		}

		lastMessage, readErr := os.ReadFile(filepath.Clean(record.lastMsgPath))
		if readErr != nil {
			record.buildErr = adapterutil.AdapterError(shared.ErrorCodeExecution, "read codex output message", readErr)
			return
		}
	...
	if record.terminal == nil {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "codex terminal execution unavailable", nil)
	}
...
func (a *Adapter) structuredOutput(lastMessage []byte, result runner.Result) (json.RawMessage, error) {
	...
	if !errors.Is(err, prompt.ErrNoAgentResult) {
		return nil, adapterutil.AdapterError(shared.ErrorCodeResult, "parse codex structured output", err)
	}
	...
	if err != nil {
		...
		return nil, adapterutil.AdapterError(shared.ErrorCodeResult, "read codex structured output", err)
	}
