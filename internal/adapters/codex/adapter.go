...
func (a *Adapter) Start(ctx context.Context, request shared.StartRequest) (*shared.Execution, error) {
	...
	if startErr != nil {
		...
		return nil, adapterError(shared.ErrorCodeExecution, "start codex exec", startErr)
	}
	...
	a.sessions[handle.ProviderSessionID] = &agentSession{
		...
		cleanup:     lastMessageResult.cleanup,
	}
	a.mu.Unlock()
...
func (a *Adapter) Resume(ctx context.Context, request shared.ResumeRequest) (*shared.Execution, error) {
	...
	if startErr != nil {
		lastMessageResult.cleanup()
		return nil, adapterError(shared.ErrorCodeExecution, "resume codex exec", startErr)
	}
...
func (a *Adapter) logPath(request shared.StartRequest) string {
	...
	return filepath.Join(a.logDir, sanitizeID(request.AttemptID)+"-codex.log")
}
...
func (a *Adapter) collectTerminal(record *agentSession) (*shared.Execution, error) {
	record.once.Do(func() {
		...
		if parseErr != nil {
			record.buildErr = adapterError(shared.ErrorCodeResult, "parse codex json output", parseErr)
			return
		}

		lastMessage, readErr := os.ReadFile(filepath.Clean(record.lastMsgPath))
		if readErr != nil {
			record.buildErr = adapterError(shared.ErrorCodeExecution, "read codex output message", readErr)
			return
		}
	...
	if record.terminal == nil {
		return nil, adapterError(shared.ErrorCodeExecution, "codex terminal execution unavailable", nil)
	}
...
// value to "." so the codex argv never carries `--cd ""` (which a cross-process
// resume without a recovered directory would otherwise produce).
...
func (a *Adapter) structuredOutput(lastMessage []byte, result runner.Result) (json.RawMessage, error) {
	structured, found, err := prompt.StructuredOutputFromText(string(lastMessage))
	if err != nil {
		return nil, adapterError(shared.ErrorCodeResult, "parse codex structured output", err)
	}
	...
	if err != nil {
		return nil, adapterError(shared.ErrorCodeResult, "read codex structured output", err)
	}
