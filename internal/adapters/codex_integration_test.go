...
func TestCodexResumeAppendsResumeArgs(t *testing.T) {
	...
	ctx := t.Context()
	start, err := adapter.Start(ctx, adapters.StartRequest{RunID: "run-1", StepID: "review", AttemptID: "attempt-1", WorkingDir: "/workspace/repo", Prompt: "do it"})
	if err != nil {
...
func (s *codexRealSessionStarter) start(_ context.Context, req runner.StartRequest) (*runner.Session, error) {
	...
	done <- runner.Result{Stdout: []byte(strings.Join([]string{
...
func (s *codexRecordingStarter) start(_ context.Context, req runner.StartRequest) (*runner.Session, error) {
	s.reqs = append(s.reqs, req)
...
func TestCodexStructuredOutputCorruptMarkerFails(t *testing.T) {
	...
	var adapterErr *adapters.Error
	if !errors.As(err, &adapterErr) {
		t.Fatalf("PollOrCollect() error type = %T, want *adapters.Error", err)
	}

	if adapterErr.Code != adapters.ErrorCodeResult {
		t.Fatalf("PollOrCollect() error code = %q, want %q", adapterErr.Code, adapters.ErrorCodeResult)
	}
}
...
func (s *codexLastMessageStarter) start(_ context.Context, req runner.StartRequest) (*runner.Session, error) {
	for i := 0; i+1 < len(req.Args); i++ {
		if req.Args[i] == "--output-last-message" {
...
// been collected and its heavy runner.Session released (record.session==nil), a
...
func TestCodexInterruptAfterTerminalDoesNotPanic(t *testing.T) {
	...
	if terminal.State != adapters.ExecutionStateSucceeded {
	...
	if interrupted.State != adapters.ExecutionStateSucceeded {
...
func TestCodexPollByRealSessionIDUsesAlias(t *testing.T) {
	...
	if again.State != adapters.ExecutionStateSucceeded || again.Handle.ProviderSessionID != "thread-123" {
...
func (s *codexFakeStarter) start(_ context.Context, req runner.StartRequest) (*runner.Session, error) {
	s.req = req
