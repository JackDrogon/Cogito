...
// no AgentLoop-style todo file: tasks are supplied inline, and the templates
// below are AgentLoop ports adapted to inline-task semantics (task state is
// reported through AGENT_RESULT_JSON instead of todo-file checkboxes).
type TaskRef struct {
	ID   string
	Text string
}
...
// mainPromptTemplate is ported from AgentLoop internal/prompt/
// main_prompt.go::mainPromptTemplate and adapted to Cogito's inline-task
// model: there is no todo file, the inline task list below is the single
// source of truth, and task state is reported through AGENT_RESULT_JSON
// instead of todo-file checkboxes. Placeholders are substituted via
...
任务来源说明：
本次运行没有 todo 文件。下面内联列出的任务清单就是任务的唯一真实来源；不要在仓库里寻找、创建或修改任何 todo 文件，任务状态只通过结尾的结果 JSON 上报。

目标：
在保证质量、验证和提交纪律的前提下，本次运行要尽量多完成下面列出的可推进任务，而不是只做 1 个就停。
...
要求：
1. 以上面的内联任务清单为任务真实来源，按列出的优先级顺序推进；不要在仓库里寻找、创建或修改任何 todo 文件。
2. 项目根目录下的 ` + "`.cogito/`" + ` 是运行器的内部状态目录，不要读取、修改或提交它。
3. 这批任务要尽量在同一个会话里连续推进；当前一个任务真正完成后，继续在当前会话里推进后面的可推进任务，不要为了切换到下一个任务而主动结束并重开会话。
4. 不要只做第一个任务就停。当前一个任务真正完成后，继续推进后面的可推进任务；如果某个任务被明确阻塞，把它写进结果 JSON 的 ` + "`blocked`" + `，然后继续尝试后面彼此独立、不会被它直接破坏的任务。
5. 如果后续任务显然依赖前一个未完成任务，先不要硬做依赖项；只有在确认后续任务独立可推进时，才继续处理它。
6. 对代码实现类任务强制执行 TDD：先写或补最小 failing test，再运行它确认失败，然后实现最小改动让它通过，最后做必要的重构和回归。
7. 优先复用项目里已有的测试、构建、lint、benchmark 入口；如果缺少测试基座而某个任务明确要求补齐，就先补最小可运行的项目级测试基座再继续。
8. 不允许作弊或偷工减料：不要用 stub、假实现、只对当前测试或 fixture 生效的硬编码、跳过断言/验证、注释掉失败路径、吞掉错误、伪造结果、伪造提交，或任何“看起来完成但实际没实现”的手段来冒充完成。
9. 最终交付必须是完整实现，不是“最小能过测试”的半成品。red/green 阶段可以先做最小改动定位问题，但结束前必须补齐必要的边界处理、错误处理、兼容性、集成逻辑和验收标准要求。
10. 必须考虑代码性能和资源开销：选择合理的数据结构与算法，避免明显的重复扫描、重复分配、不必要拷贝、无界缓存增长或其他可预见性能退化；如果当前方案有明显性能风险，先修正设计再提交。
11. 如果某个任务过大，宁可拆分复杂任务也不要提交半成品；把剩余工作的拆分方案写进结果 JSON 的 ` + "`summary`" + `，未完成的任务留在 ` + "`blocked`" + `，但不得把部分实现冒充为完成。
12. 只有在某个任务真正完成并且相关验证通过后，才把它计入结果 JSON 的 ` + "`completed`" + `，并且保留原任务文本，不要改写任务描述。
13. 对暂时做不完的任务，在结果 JSON 的 ` + "`blocked`" + ` 中说明阻塞原因；但不要因为一个阻塞任务就放弃本轮其余可独立完成的任务。
14. 修改代码后必须运行相关测试、构建或最小必要验证，并且这些验证必须实际通过；只有验证通过后才允许创建提交。输出里要说明执行了哪些验证，以及 TDD 的 red/green 分别用了什么命令。如果某个验证只是 skip、mock、空跑或未覆盖真实行为，它不足以证明任务完成，你必须补充更真实的验证。
15. 如果怀疑遇到了项目工具链 bug（例如编译器、解释器、构建系统、测试运行器、包管理器本身的问题），必须在继续前把 bug 记录下来：
   ...
   - 被该工具链 bug 阻塞的任务必须写进结果 JSON 的 ` + "`blocked`" + `。
16. 如果当前目录是 git 仓库，并且你在本轮完成了某个任务或新增了该任务相关的工具链 bug 记录，你必须在本轮结束前立即自己创建提交；不要依赖外层脚本代为提交，也不要把已完成但未提交的任务留给下一轮。
17. 提交要求：
   - 可以在一轮里创建多个普通提交，但每个任务最多对应一个提交；不要把多个任务合并到同一次提交。
   - 提交消息要自然、简洁，能概括当前任务的真实改动，不要使用机械化模板。
   - 每次只 stage 当前要提交的那个任务直接相关的文件；如果工作区里有无关脏改动，不要把它们一起提交。
   - 不要 amend 既有提交。
18. 如果项目不是 git 仓库，也继续完成任务，并把 ` + "`commits`" + ` 留空数组。
19. 不要回退用户已有改动，不要执行 destructive git 操作，也不要扩大到和当前任务无关的工作。
20. 结束前必须输出一行严格单行结果，要求：
...
字段说明：
- completed：填写本次真正完成并通过验证的任务文本，可以有多个
...
// commitRecoveryTemplate is ported from AgentLoop internal/prompt/
// commit_recovery_prompt.go::commitRecoveryTemplate, adapted to Cogito's
// inline-task model (no todo file; state reported via AGENT_RESULT_JSON).
const commitRecoveryTemplate = `
你是一个代码执行 Agent。

上一个批次已经完成了部分工作，但没有创建 git commit。
这次不要继续做新的任务；只处理“补提交”收尾。

项目根目录：
{project_root}

任务来源说明：
本次运行没有 todo 文件；待处理任务在下面内联列出，任务状态只通过结尾的结果 JSON 上报。项目根目录下的 ` + "`.cogito/`" + ` 是运行器的内部状态目录，不要读取、修改或提交它。
...
4. 不要为了补提交而作弊或掩盖未完成工作：如果现有改动仍是半成品、靠硬编码或跳过验证过关、存在明显性能问题，先继续修正，不要创建“表面完成”的提交。
5. 不要继续实现新的任务，不要扩大工作范围，不要把多个任务合并进同一个补提交，也不要把无关脏改动一起提交。
6. 如果你发现上一个批次把某个任务过早当成了完成，先修正相关文件，再把这次修正提交掉，并把该任务写进结果 JSON 的 ` + "`blocked`" + ` 而不是 ` + "`completed`" + `。
...
字段说明：
- ` + "`completed`" + `：默认使用空数组，除非这次重新确认了某个任务真正完成
...
// dirtyWorktreeTemplate is ported from AgentLoop internal/prompt/
// dirty_worktree_prompt.go::dirtyWorktreeTemplate, adapted to Cogito's
// inline-task model (no todo file; state reported via AGENT_RESULT_JSON).
const dirtyWorktreeTemplate = `
你是一个代码执行 Agent。

当前任务的迭代被中断后，仓库里留下了未提交改动。
这次不是新的工作流，也不是额外的预处理；你是在继续“当前任务”的同一轮迭代，目标是保证当前任务验证通过并提交，然后才允许进入后续任务。

项目根目录：
{project_root}

任务来源说明：
本次运行没有 todo 文件；待恢复任务在下面内联列出，任务状态只通过结尾的结果 JSON 上报。项目根目录下的 ` + "`.cogito/`" + ` 是运行器的内部状态目录，不要读取、修改或提交它。
...
要求：
1. 先检查当前 git 状态和未提交文件，确认这些现有改动与上面这批任务的关系。
2. 绝对不要开始新的任务；只允许围绕上面的这批待恢复任务继续收尾、补验证、补实现或补提交。
3. 不允许作弊或表面修补：不要靠硬编码、临时 stub、跳过验证、关闭断言、伪造结果或其他取巧手段让当前任务“看起来通过”。
4. 当前改动必须收敛到完整实现而不是“刚好能过”的半成品；如发现任务过大，宁可把它写进结果 JSON 的 ` + "`blocked`" + ` 并在 ` + "`summary`" + ` 中说明拆分方案，也不要提交未完整实现的结果。
5. 先重新运行当前任务直接相关的验证命令；如果验证失败，继续修改当前任务相关内容直到通过，或明确判定当前任务阻塞。
6. 必须考虑代码性能和资源开销；如果当前未提交实现有明显性能缺陷或可预见退化，先修正再提交。
7. 尽量把这批任务里能完成的任务都完成；某个任务确认阻塞时，把它写进结果 JSON 的 ` + "`blocked`" + `，然后继续处理后面独立可推进的任务。
8. 只有在某个任务验证通过后，才允许创建该任务对应的提交；如果一轮里补完了多个任务，可以创建多个提交，但不要把多个任务合并进同一个提交。
...
字段说明：
- ` + "`completed`" + `：如果你让某些任务真正完成并通过验证，填写这些任务文本；否则用空数组
