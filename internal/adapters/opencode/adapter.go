...
func (a *Adapter) Start(ctx context.Context, request shared.StartRequest) (*shared.Execution, error) {
	...
	if startErr != nil {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "start opencode session", startErr)
	}
...
func interruptFromTerminal(terminal *shared.Execution) (*shared.Execution, error) {
	if terminal == nil {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "opencode execution session not found", nil)
	}
...
func (a *Adapter) Resume(ctx context.Context, request shared.ResumeRequest) (*shared.Execution, error) {
	...
	if startErr != nil {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "resume opencode session", startErr)
	}
...
func (a *Adapter) binaryPath() (string, error) {
	...
	for _, name := range binaryCandidates {
		...
		return "", adapterutil.AdapterError(shared.ErrorCodeExecution, fmt.Sprintf("locate %s binary", name), err)
	}
	...
	return "", adapterutil.AdapterError(shared.ErrorCodeExecution, "opencode binary not found", lastErr)
}
...
func (a *Adapter) logPath(request shared.StartRequest) string {
	...
	return filepath.Join(a.logDir, adapterutil.SanitizeID(request.AttemptID)+"-opencode.log")
}
...
func (a *Adapter) lookupSession(handle shared.ExecutionHandle) (*agentSession, error) {
	...
	if record.handle.RunID != handle.RunID || record.handle.StepID != handle.StepID || record.handle.AttemptID != handle.AttemptID {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "opencode execution handle does not match session", nil)
	}
...
func (a *Adapter) collectTerminal(record *agentSession) (*shared.Execution, error) {
	record.once.Do(func() {
		...
		if parseErr != nil {
			record.buildErr = adapterutil.AdapterError(shared.ErrorCodeResult, "parse opencode json output", parseErr)
			return
		}
	...
	if record.terminal == nil {
		return nil, adapterutil.AdapterError(shared.ErrorCodeExecution, "opencode terminal execution unavailable", nil)
	}
...
func (a *Adapter) structuredOutput(outputText string, result runner.Result) (json.RawMessage, error) {
	...
	if !errors.Is(err, prompt.ErrNoAgentResult) {
		return nil, adapterutil.AdapterError(shared.ErrorCodeResult, "parse opencode structured output", err)
	}
	...
	if err != nil {
		...
		return nil, adapterutil.AdapterError(shared.ErrorCodeResult, "read opencode structured output", err)
	}
