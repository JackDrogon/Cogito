...
func (a *Adapter) Start(ctx context.Context, request shared.StartRequest) (*shared.Execution, error) {
	...
	if err != nil {
		return nil, adapterError(shared.ErrorCodeExecution, "run opencode session", err)
	}
...
func (a *Adapter) Interrupt(_ context.Context, _ shared.ExecutionHandle) (*shared.Execution, error) {
	...
	return nil, adapterError(shared.ErrorCodeExecution, "opencode interrupt is not implemented", nil)
}
...
func (a *Adapter) Resume(_ context.Context, _ shared.ResumeRequest) (*shared.Execution, error) {
	...
	return nil, adapterError(shared.ErrorCodeExecution, "opencode resume is not implemented", nil)
}
...
func buildRunArgs(request shared.StartRequest) []string {
	prompt := strings.TrimSpace(request.Prompt)
	if prompt == "" {
		prompt = fmt.Sprintf("run %s/%s", request.StepID, request.AttemptID)
	}

	return []string{"run", "--json", prompt}
}
