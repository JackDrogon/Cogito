...
// Starter launches the claude agent asynchronously. It defaults to provider.StartProcess
...
func interruptFromTerminal(terminal *provider.Execution) (*provider.Execution, error) {
	if terminal == nil {
		return nil, provider.NewError(provider.ErrorCodeExecution, "claude execution session not found", nil)
	}
...
func (a *Adapter) binaryPath() (string, error) {
	...
	if errors.Is(err, exec.ErrNotFound) {
		return "", provider.NewError(provider.ErrorCodeExecution, "claude binary not found", err)
	}

	return "", provider.NewError(provider.ErrorCodeExecution, "locate claude binary", err)
}
...
func (a *Adapter) logPath(request provider.StartRequest) string {
	...
	return filepath.Join(a.logDir, provider.SanitizeID(request.AttemptID)+"-claude.log")
}
...
func (a *Adapter) lookupSession(handle provider.ExecutionHandle) (*agentSession, error) {
	...
	if record.handle.RunID != handle.RunID || record.handle.StepID != handle.StepID || record.handle.AttemptID != handle.AttemptID {
		return nil, provider.NewError(provider.ErrorCodeExecution, "claude execution handle does not match session", nil)
	}
...
func (a *Adapter) collectTerminal(record *agentSession) (*provider.Execution, error) {
	record.once.Do(func() {
		...
		if parseErr != nil {
			record.buildErr = provider.NewError(provider.ErrorCodeResult, "parse claude json output", parseErr)
			return
		}
	...
	if record.terminal == nil {
		return nil, provider.NewError(provider.ErrorCodeExecution, "claude terminal execution unavailable", nil)
	}
...
func (a *Adapter) structuredOutput(response *response, result provider.ProcessResult) (json.RawMessage, error) {
	...
	if !errors.Is(err, prompt.ErrNoAgentResult) {
		return nil, provider.NewError(provider.ErrorCodeResult, "parse claude structured output", err)
	}
	...
	if err != nil {
		...
		return nil, provider.NewError(provider.ErrorCodeResult, "read claude structured output", err)
	}
