...
func TestBinaryPathMissingIsExplicit(t *testing.T) {
	...
	_, err := adapter.Start(t.Context(), provider.StartRequest{RunID: "run-1", StepID: "review", AttemptID: "attempt-1", Prompt: "Fix it"})
	if err == nil {
...
func TestProviderSessionIDReturnsLastThreadID(t *testing.T) {
	request := provider.StartRequest{StepID: "step", AttemptID: "attempt"}
	events := []event{
...
func TestProviderSessionIDFallsBackToSynthetic(t *testing.T) {
	request := provider.StartRequest{StepID: "review", AttemptID: "attempt-1"}
