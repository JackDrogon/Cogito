...
// provider.StartProcess and is injectable so tests can substitute a fake supervisor.
...
func (a *Adapter) Start(ctx context.Context, request provider.StartRequest) (*provider.Execution, error) {
	...
	session, startErr := a.starter(ctx, provider.ProcessRequest{
		...
		PIDFile:       provider.PIDFilePath(a.logDir, request),
		Label:         provider.ProcessLabel(request),
		ExtraSink:     a.liveSink,
		Timeout:       a.timeout,
		IdleTimeout:   a.idle,
	})
	if startErr != nil {
		return nil, provider.NewError(provider.ErrorCodeExecution, "start opencode session", startErr)
	}
...
func interruptFromTerminal(terminal *provider.Execution) (*provider.Execution, error) {
	if terminal == nil {
		return nil, provider.NewError(provider.ErrorCodeExecution, "opencode execution session not found", nil)
	}
...
func (a *Adapter) Resume(ctx context.Context, request provider.ResumeRequest) (*provider.Execution, error) {
	...
	session, startErr := a.starter(ctx, provider.ProcessRequest{
		...
		PIDFile:       provider.PIDFilePath(a.logDir, resumeStart),
		Label:         provider.ProcessLabel(resumeStart),
		ExtraSink:     a.liveSink,
		Timeout:       a.timeout,
		IdleTimeout:   a.idle,
	})
	if startErr != nil {
		return nil, provider.NewError(provider.ErrorCodeExecution, "resume opencode session", startErr)
	}
...
func (a *Adapter) binaryPath() (string, error) {
	...
	for _, name := range binaryCandidates {
		...
		return "", provider.NewError(provider.ErrorCodeExecution, fmt.Sprintf("locate %s binary", name), err)
	}
	...
	return "", provider.NewError(provider.ErrorCodeExecution, "opencode binary not found", lastErr)
}
...
func (a *Adapter) binaryVersion(ctx context.Context, binaryPath string) string {
	a.versionOnce.Do(func() {
		...
		if err != nil {
			...
			slog.Warn("opencode: version probe failed", "err", err)
...
func (a *Adapter) logPath(request provider.StartRequest) string {
	...
	return filepath.Join(a.logDir, provider.SanitizeID(request.AttemptID)+"-opencode.log")
}
...
func (a *Adapter) lookupSession(handle provider.ExecutionHandle) (*agentSession, error) {
	...
	if record.handle.RunID != handle.RunID || record.handle.StepID != handle.StepID || record.handle.AttemptID != handle.AttemptID {
		return nil, provider.NewError(provider.ErrorCodeExecution, "opencode execution handle does not match session", nil)
	}
...
func (a *Adapter) collectTerminal(record *agentSession) (*provider.Execution, error) {
	record.once.Do(func() {
		...
		if parseErr != nil {
			record.buildErr = provider.NewError(provider.ErrorCodeResult, "parse opencode json output", parseErr)
			return
		}
	...
	if record.terminal == nil {
		return nil, provider.NewError(provider.ErrorCodeExecution, "opencode terminal execution unavailable", nil)
	}
...
// dropped (N2).
func (a *Adapter) structuredOutput(outputText string, result provider.ProcessResult) (json.RawMessage, error) {
	structured, err := prompt.StructuredOutputFromText(outputText)
	if err == nil {
		return structured, nil
	}

	if !errors.Is(err, prompt.ErrNoAgentResult) {
		return nil, provider.NewError(provider.ErrorCodeResult, "parse opencode structured output", err)
	}
	...
	if err != nil {
		...
		return nil, provider.NewError(provider.ErrorCodeResult, "read opencode structured output", err)
	}
