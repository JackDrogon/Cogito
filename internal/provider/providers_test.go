	...
	RunContractSuite(t, []ContractCase{
		{
			...
			Provider:         contractFakeAdapter(capabilities),
...
func TestUnsupportedCapabilitiesAreExplicit(t *testing.T) {
	tests := []struct {
		name           string
		call           func(adapter Provider) error
...
func contractFakeAdapter(capabilities CapabilityMatrix) Provider {
	return NewFakeProvider(FakeConfig{
