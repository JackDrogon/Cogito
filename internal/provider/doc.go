/*
Package provider defines Cogito's provider SPI.
...
The SPI is centered on Provider plus a small registry used by higher-level wiring.
...
CapabilityMatrix makes optional provider features explicit. Runtime and tests can
...
shapes separate lets providers preserve provider details during polling and then
emit a narrower, workflow-safe result at the normalization boundary.

# Registry role

Register, Lookup, and RegisteredNames provide a process-local catalog of provider
implementations. The registry exists to keep CLI wiring simple: providers self-
...
FakeProvider and RunContractSuite codify the SPI behavior expected of every real
provider implementation. Contract tests protect behavioral parity across providers
...
Individual provider implementations are responsible for any additional internal
