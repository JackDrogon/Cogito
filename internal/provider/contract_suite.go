...
func runCapabilityTest(t *testing.T, tc *ContractCase) CapabilityMatrix {
	...
	capabilities := tc.Provider.DescribeCapabilities()
...
func runStartAndPollTest(t *testing.T, ctx context.Context, tc *ContractCase) {
	...
	execution, err := tc.Provider.Start(ctx, tc.StartRequest)
	if err != nil {
	...
	for i, wantState := range tc.WantPollStates {
		execution, err = tc.Provider.PollOrCollect(ctx, execution.Handle)
		if err != nil {
	...
	result, err := tc.Provider.NormalizeResult(ctx, normalizeRequest)
	if err != nil {
...
func runInterruptTest(t *testing.T, ctx context.Context, tc *ContractCase) {
	...
	interruptExecution, err := tc.Provider.Start(ctx, *tc.InterruptRequest)
	if err != nil {
		t.Fatalf("Start() for interrupt error = %v", err)
	}

	interruptExecution, err = tc.Provider.Interrupt(ctx, interruptExecution.Handle)
	if err != nil {
...
func runResumeTest(ctx context.Context, params resumeTestParams) {
	...
	resumeExecution, err := params.ContractCase.Provider.Start(ctx, *params.ContractCase.ResumeRequest)
	if err != nil {
		params.Test.Fatalf("Start() for resume error = %v", err)
	}

	if params.Capabilities.Interrupt {
		resumeExecution, err = params.ContractCase.Provider.Interrupt(ctx, resumeExecution.Handle)
		if err != nil {
			params.Test.Fatalf("Interrupt() before resume error = %v", err)
		}
	}

	resumeExecution, err = params.ContractCase.Provider.Resume(ctx, ResumeRequest{Handle: resumeExecution.Handle})
	if err != nil {
	...
	for i, wantState := range params.ContractCase.WantResumePollStates {
		resumeExecution, err = params.ContractCase.Provider.PollOrCollect(ctx, resumeExecution.Handle)
		if err != nil {
	...
	if params.ContractCase.WantResumeResult != nil {
		...
		result, err := params.ContractCase.Provider.NormalizeResult(ctx, resumeNormalize)
		if err != nil {
