...
|-- provider/  # provider SPI, registry, fake provider, provider impls
...
| Add a new provider | `internal/provider/` | see `internal/provider/AGENTS.md`; use contract suite |
...
- Tests live beside packages and use Go's standard `testing` package; providers are the only area with a reusable contract suite.

## ANTI-PATTERNS
- Do not move shared behavior into `internal/app/` just because the CLI reaches it first.
- Do not let `runtime/` know provider details beyond the provider SPI.
...
## NOTES
- Highest-complexity domains are `runtime/`, `app/`, and `provider/`; read package docs before changing them.
